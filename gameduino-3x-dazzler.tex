...
\begin{tikzpicture}[
  ...
  \node [rightnode, right=of FPGA] (CONSOLE) {CONSOLE};
  \node [rightnode, below=of CONSOLE] (UART) {UART};
  \node [rightnode, below=of UART] (WII) {\textbf{WII}};
  \node [rightnode, below=of WII] (HDMI) {HDMI};
  \node [rightnode, below=of HDMI] (SD) {\textbf{SD}};
  ...
  \draw[<->, ultra thick] (FPGA.east |- WII.west) -- (WII.west);
  \draw[->, thick, double] (FPGA.east |- HDMI.west) -- (HDMI.west);
  \draw[<->, ultra thick] (FPGA.east |- SD.west) -- (SD.west);

  \draw[<-, thick] (FPGA.west |- OSC 6 MHz.east) -- (OSC 6 MHz.east);
  \draw[<->, thick, double] (FPGA.west |- SPI.east) -- (SPI.east);
